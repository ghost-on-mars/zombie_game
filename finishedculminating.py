...
bgimage.place(x = -2,y = 0)#relx=-0.1, rely=0.0, anchor ='nw')#relx=1.0, rely=0.0, anchor ='ne')#x = 0,y = 0)  #positions it
...
def start():
    ...
    #mergedsearchables.place(x=150, y=150)


    #rdoorhuh = tk.Label(image=rdoory)
    #rdoorhuh.place(x=200, y=150)

    cratelock = 'no'
    #if roomvar > 1:
       # print((noisebar['value']))

    if 'tonextroom' in inventory: #in case tonextroom gto added it removes it
        inventory.remove('tonextroom')
        print('get taht out of here buster')
    ...
    def nextroom():
        ...
        print('rdoor thing', rdoorcounter, 'rdoor thing')
    ...
    def openinventory():
        ...
        #inventoryframe.place(relx=0.92, rely=0.1, anchor=tk.CENTER)
        ...
        #spacething=tk.Label(inventoryframe)
        #spacething.grid(row=1, column=0, padx=70, pady=5)

        '''item1=tk.Label(inventoryframe, text='')
        item1.grid(row=1, column=0, padx=5, pady=5)
        item2=tk.Label(inventoryframe)
        item2.grid(row=1, column=1, padx=5, pady=5)
        item3=tk.Label(inventoryframe)#, text=str(inventory[2]))
        item3.grid(row=1, column=2, padx=10, pady=5)
        item4=tk.Label(inventoryframe)#, text=str(inventory[3]))
        item4.grid(row=2, column=0, padx=5, pady=5)
        item5=tk.Label(inventoryframe)#, text=str(inventory[4]))
        item5.grid(row=2, column=1, padx=5, pady=5)
        item6=tk.Label(inventoryframe)#, text=str(inventory[5]))'''
        ...
        def buttonsetup():
            ...
            if len(inventory) > 1:
                ...
                item2.config(text=str(inventory[1]))#item2=tk.Label(inventoryframe, text=str(inventory[1]))
                #item2.grid(row=1, column=1, padx=5, pady=5)
            if len(inventory) > 2:
                item3.grid(row=1, column=2, padx=10, pady=5)
                item3.config(command=lambda b=inventory[2]: destroy_button(b))
                item3.config(text=str(inventory[2]), padx=5)#item3=tk.Label(inventoryframe, text=str(inventory[2]))
                #item3.grid(row=1, column=2, padx=5, pady=5)
            if len(inventory) > 3:
                item4.grid(row=2, column=0, padx=5, pady=5)
                item4.config(command=lambda b=inventory[3]: destroy_button(b))
                item4.config(text=str(inventory[3]))#=tk.Label(inventoryframe, text=str(inventory[3]))
                #item4.grid(row=2, column=0, padx=5, pady=5)
            if len(inventory) > 4:    
                item5.grid(row=2, column=1, padx=5, pady=5)
                item5.config(command=lambda b=inventory[4]: destroy_button(b))
                item5.config(text=str(inventory[4]))#=tk.Label(inventoryframe, text=str(inventory[4]))
                #item5.grid(row=2, column=1, padx=5, pady=5)
            if len(inventory) > 5: 
        ...
        def inventorysetup():
            ...
            for i in range(len(inventory)):
                ...
                if 'screws' in inventory[maybescrews]:
                    print(howmanyscrews, 'howmanyscrews')
                    ...
                    print(inventory[maybescrews], 'inventoruy maybe screws')
                    if howmanyscrews >=2: #if theres more than 2 screwss thing in the list
                        print('removing ')#, inventory(maybescrews))
            ...
            for i in range(len(inventory)):
                ...
                if 'bullets' in inventory[maybebullets]:
                    print(howmanybullets, 'howmanybullets')
                    ...
                    print(inventory[maybebullets], 'inventoruy maybe bullets')
                    if howmanybullets >=2: #if theres more than 2 bulletss thing in the list
                        print('removing ')#, inventory(maybebullets))
                        del inventory[maybebullets]
                        maybebullets-=1 #so it doesnt keep goiing too long
            
                        print(inventory)
                    #inventory.remove[len(inventory)] #removes the one that was just added
            buttonsetup()
                    #(item+(maybebullets+1)).config(text='this worked?????????')
        
                    #typedcode.replace(typedcode[-1], '')


        def destroy_button(itemininventory):
            global screwcounter
            global bulletcounter
            #itemininventory.destroy()
            print(inventory)
    ...
    def searchunlockedcrate(): #once a crate is unlocked it is a much better chance to get good things than the normal crates
        ...
        if itemfound != '2 bullets' and itemfound != '3 bullets' and itemfound != '6 screws' and itemfound != '2 screws' and itemfound != 'nothing': #this is all just so it only has a when its grammatically correct. so it says 'a bat' but not 'a 6 screws'
            print('You found a ' + itemfound + ' in the crate') #print temporarily, make it a label eventually
            ...
            print('You found ' + itemfound + ' in the crate') #print temporarily, make it a label eventually
        ...
        #thinglabel.config(text='There was a paper in its pocket\nwith ' + foundcode + ' written on it')
    ...
    def lockedcrate():
        ...
        def breakopenBat(): #if the bat is used, this tracks it 
            global usingbat 
            usingbat=1 
            print('bat useddddddddddddddddd')
            breakopen()
        ...
        #Xbutton.place(relx=0.92, rely=0.25, anchor=tk.CENTER)
    ...
    def shelfcratesearch():
        ...
        global screwcounter
        global progress
        print('shelfcratesearch')
        possibleitems = ['bat', 'gun', '2 bullets', '3 bullets', '6 screws', '2 screws', 'crowbar', 'grey key', 'rusty key', 'plank', 'nothing'] #screws and plank are useless. COULD JUST REMOVE THE LIST SIMCE I CAN JJUST PUT THE ITEM NAMES IN THE RADNOM PART
        #itemfound = random.choice(possibleitems)
        #itemfound = random.choices(['bat', 'gun', '2 bullets'], p =[ 0.1, 0.15, 0], size=(100))
        ...
        if itemfound != '2 bullets' and itemfound != '3 bullets' and itemfound != '6 screws' and itemfound != '2 screws' and itemfound != 'nothing': #this is all just so it only has a when its grammatically correct. so it says 'a bat' but not 'a 6 screws'
            ...
            print('You found a ' + itemfound) #print temporarily, make it a label eventually
            ...
            if 'bullets' in itemfound:
                ...
                #itemfound=(str(bulletcounter) + ' bullets')
                print(str(bulletcounter) + ' bullets')
                inventorylen = int(len(inventory))
                print(inventorylen, 'inventoreylen')
            ...
            if 'screws' in itemfound:
                ...
                #itemfound=(str(screwcounter) +' screws')
                print(str(screwcounter) +' screws')

            print('You found ' + itemfound) #print temporarily, make it a label eventually
    ...
    def doorsearch():
        ...
        if dooritem != 'tonextroom' and dooritem != 'nothing':
            ...
            print(inventory, len(inventory))
            noisebar['value']+=10
            print('It was a storage closet. You found a ' + dooritem)
            ...
            print('nothinh')
            print('It was a storage closet. You found ' + dooritem)
    ...
    attackresult=0
    searchableslist = [shelf, crate]#maybe adda  blank image so it can be less than three?
    #just make it one door so there cant be duplicates of the same door
    doorslist = [ldoor, rdoor]
    chosenlist = []
    realchosenlist = []
    #left door pyiamge2, right door pyimahe3, shelf pyimage4, crate pyimage5.
    

    image1 = random.choice(searchableslist)
    #if image1 == ldoor or image1 == rdoor: #so there cant be duplicate doors
        #searchableslist.remove(image1)\
    print(searchableslist, image1, 'first random image')
    ...
    if image2 == ldoor or image2 == rdoor: #so there cant be duplicate doors
        print('removing ' + str(image2) + ' as an option')
        searchableslist.remove(image2)
        print(searchableslist)
        #image2 = random.choice(searchableslist) #reselects a new random thing
    print(str(image2))
    image3 = random.choice(searchableslist)
    print(image3)

    if image3 == crate:
        print('image 3 is crate?')
        cratelock = str(random.choices((crate, 'lockedcrate'), weights=(1, 100), k=1)) #60 40 split
        print(image3, cratelock)
    ...
    '''if rdoor in chosenlist:
        print('theres a right door!')
        
        rdoorposition.place(x=310, y=118)
    else:
        print('no right doro')
        if roomvar > 1:
            print('still no right doro')
            rdoorposition.place(x=1000, y=1000)
            rdoorposition.destroy()
    if ldoor in chosenlist:
        print('theres a left door!')
        
        ldoorposition.place(x=171, y=118)
    else:
        print('no left door')
        if roomvar > 1:
            print('still no left door')
            ldoorposition.place(x=1000, y=1000)#_forget()'''
    ...
    print('chosenlistreallllllll', realchosenlist)

    #MAYBE JUST MAKE IMG 1 ALWAYS SHELF 1, IMG2 ALWAYS SHELF 2, IMG3 ALWAYS SHELF3, AND ASME FOR CRATES
    searchableitem1 = tk.Label(root, text=" ", image=image1, borderwidth=0, highlightthickness=0) 
    #searchableitem1.grid(column=2, row=2, padx=0, pady=0)
    searchableitem1.place(x=50, y=115)
    if image1 == rdoor:
        print('image 1 is right door')
        searchableitem1.place(x=307, y=117)
    if image1 == ldoor:
        print('image 1 is left door')
        searchableitem1.place(x=160, y=117)
    if image1 == shelf:
        print('image 1 is a shelf')
        img1shelf = random.choice((shelflist))
        print(shelflist, 'shelfglist')
        shelflist.remove(img1shelf)
        realchosenlist.append(img1shelf)
        print(shelflist, 'shelflist afetr removal')
        ...
        if img1shelf==shelf3:
            ...
            #searchableitem1.place(x=429, y=161)

    if image1 == crate:
        print('image 1 is a crate')
        img1crate = random.choice((cratelist))
        print(cratelist, 'crateglist')
        cratelist.remove(img1crate)
        realchosenlist.append(img1crate)
        print(cratelist, 'cratelist afetr removal')
    ...
    searchableitem2.place(x=437, y=271)#.grid(column=4, row=2, padx=0, pady=0)
    if image2 == rdoor:
        print('image 2 is right door')
        searchableitem2.place(x=307, y=117)
    if image2 == ldoor:
        searchableitem2.place(x=160, y=117)
    if image2 == shelf:# and image1 == shelf:
        print('image 2 is a shelf')
        ...
        if img2shelf==shelf3:
            ...
            #searchableitem2.place(x=429, y=161)
    if image2 == crate:
        print('image 2 is a crate')
    ...
    if image3 == shelf:
        ...
        if img3shelf==shelf3:
            ...
            #searchableitem3.place(x=429, y=161)

    if image3 == crate:
        print('image 3 is a crate')
        #if img2shelf==shelf2 or img1shelf==shelf2 or img3shelf==shelf2:
    ...
    print('merged searchablkes time!')
    print('real cgosen list', realchosenlist)
    #mergedimg=tk.PhotoImage(file='crate1.png') #the crate 1 is just a placeholder
    
    if crate3 in realchosenlist:
        print('crate three is herrrrrreeeeeeeeeeeeeeeeeeee')
    ...
    if shelf3 in realchosenlist and crate3 in realchosenlist: 
        print('shelf 3 and crate 3')
        mergedimg.config(file='crate3shelf3.png')
        mergedsearchables.place(x=447, y=187)
    #if crate2 in realchosenlist and crate3 in realchosenlist:
       # print('carte 2 and 3') 
       # mergedimg.config(file='crate2.png')
        #mergedsearchables.place(x=415, y=249)
       # if 
    ...
    noiselabel = tk.Label(noiseframe, text='Noise level')#, font=('Arial', 16))
    noiselabel.grid(row=0, column=0, columnspan=5, pady=(2, 5))
    #noiselabel.place(relx=0.92, rely=0.1, anchor=tk.CENTER)

    #noisebar = ttk.Progressbar(noiseframe, maximum=100)
    ...
    if cratelock == "['lockedcrate']":
        ...
        cratelock='no' #this maybe fixes the error where it this if is true when it shouldnt be and it says crate button is not defined



        #cratebutton = tk.Button(searchframe, text='lockedCrate', font=('Arial', 12), command=lockedcrate)
        #cratebutton.grid(row=5, column=0, pady=5)
    ...
    def processattackresult():
        ...
        if attackresult == 'It hurt the zombie a lot':
            ...
            zhealthbar['value']-=2
            print(zombiehealth)
            print('that did a little but the thing is being dumb')
    ...
    if noisebar['value']>=70: #THIS NEEDS TO BE MOVED 
        ...
        print('ahhhhhh youve been caught caus eyou were too loud :(')
        ...
        #healthbar['value']=healthvalue #this is so it doesnt reset the health every time start() is triggered by going to a new room. so this sets the value of the progress bar to what it was before the next room
...
#maybe make the button look nice by drawing one and using the image instead of an actual button
